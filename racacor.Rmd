...
porte_bolsa <- merge(porte, pop, "Ibge")
```
...
porte_praticas[, c("Taxa_Antropometria", "Taxa_Praticas", "Taxa_Vacinacao")] <- porte_praticas %>%
  summarise(
...
```{r}
porte_praticas %>%
  ggplot(aes(x= porte_ibge, y = Antropometria, fill = porte_ibge)) +
  geom_boxplot() +
  scale_fill_manual(
    values = cores_porte
  ) +
